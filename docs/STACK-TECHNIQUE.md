...
│  · proxy /api/* → backend Render (rewrite 200)                   │
└──────────────────────────┬──────────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────────┐
...
| html5-qrcode | 2.3 | Scan de QR code par la caméra (validation de séance) |
| react-qr-code | 2.2 | Génération du QR code présenté par le client |
| three | 0.184 | Animation 3D de la page d'accueil |
| react-hot-toast | 2.6 | Notifications toast |
| Vitest + Testing Library | 3 / 16 | Tests de composants (dev) |
...
| google-auth-library | 10 | Vérification des ID tokens Google Sign-In |
| stripe | 21 | Paiements |
...
| Jest + Supertest | 30 / 7 | Tests unitaires, API et intégration |
...
**Prisma** comme ORM :
- **Schéma déclaratif unique** (`schema.prisma`, 18 modèles) qui sert à la fois de documentation du modèle de données, de source des migrations et de générateur de client typé.
...
- **Alternatives écartées** : SQL brut (trop d'écriture répétitive pour 18 modèles), Sequelize (API datée), TypeORM (fiabilité des migrations).
...
- **Google Sign-In** (`POST /auth/google`) : le frontend obtient un *ID token*, le backend le vérifie avec `google-auth-library` contre `GOOGLE_CLIENT_ID`, puis rattache ou crée le compte. Troisième porte d'entrée, même sortie — les trois mécanismes convergent vers la même paire de JWT.
...
Pièces d'identité, diplômes, avatars et photos de galerie. Les trois middlewares d'upload utilisent **`multer.memoryStorage()`** : le fichier ne touche jamais le disque, ses octets sont écrits directement en colonne `bytea` PostgreSQL et servis par une route `/api` dédiée. Limites : 5 Mo, PDF/JPG/PNG uniquement ; la lecture des pièces justificatives est réservée à l'ADMIN.

**Pourquoi la base et non le disque** : le système de fichiers de l'offre gratuite Render est **éphémère** — effacé à chaque déploiement et à chaque réveil de veille. Tout document téléversé y disparaîtrait. Stocker des binaires en base n'est pas idéal (poids des sauvegardes, pas de CDN) : c'est une solution de contournement assumée, à remplacer par un stockage objet (S3/R2) dès que le volume le justifiera.

### Tests — Jest, Vitest, Playwright

Cinq niveaux, 1 236 tests (détail complet dans [STRATEGIE-TESTS.md](STRATEGIE-TESTS.md)) :

- **Jest** (backend) — trois projets déclarés dans `jest.config.js` : `unit` (Prisma, Redis, Stripe et Resend simulés), `api` (**Supertest** sur l'app Express réelle, infrastructure simulée) et `integration` (vraie base PostgreSQL dédiée, exécution séquentielle).
- **Vitest + Testing Library** (frontend) — utilitaires, client HTTP, contexte d'authentification, composants de réservation, en environnement jsdom.
- **Playwright** (`e2e/`) — parcours utilisateurs complets dans Chromium, contre une pile réelle sur ports dédiés (API 3100, front 5199) et une base `goupyl_sport_e2e`.

`.github/workflows/tests.yml` rejoue les cinq niveaux à chaque poussée et chaque pull request.
...
|---|---|
| Binaires (documents, avatars, photos) stockés en base | Stockage objet S3/R2 : alléger les sauvegardes et servir via CDN |

...
Four levels, ~1 236 tests. Full rationale, conventions and known defects: **`docs/STRATEGIE-TESTS.md`**; functional test plan: **`docs/CAHIER-RECETTE.md`**.

```bash
# backend/  (jest.config.js declares three projects: unit | api | integration)
npm test                  # unit + api — no external dependency, ~6 s
npm run test:unit         # services, validators, middlewares, utils (Prisma/Redis/Stripe/Resend mocked)
npm run test:api          # real Express app over HTTP (supertest), infra mocked
npm run test:db:setup     # ⚠️ once: creates goupyl_sport_test + pushes schema
npm run test:integration  # real PostgreSQL, --runInBand
npm run test:all          # the three levels
npm run test:coverage     # thresholds enforced in jest.config.js (branches ≥ 45 %)
npx jest --selectProjects unit -t "quota"     # single concern

# frontend/  (vitest + @testing-library/react, jsdom)
npm test                  # 169 tests
npm run test:coverage

# e2e/  (Playwright, real browser against the whole stack)
npm run setup             # ⚠️ once: creates goupyl_sport_e2e + installs chromium
npm test                  # 59 user journeys, ~90 s
npm run test:headed       # visible browser (demo)
```

Non-obvious constraints, all handled by the helpers — do not undo them:
- **`src/app.js` calls `dotenv.config()`**, which loads the *development* `.env`. dotenv never overrides an already-set variable but does fill in missing ones, so `delete process.env.RESEND_API_KEY` gets silently undone and tests would send **real emails**. `tests/setup/env*.js` therefore set **empty strings**, never delete.
- **Rate limiting** (100 req/min/IP, 10/min on auth routes) breaks any suite running from one machine. Tests simulate a distinct client IP per request via `X-Forwarded-For` (the app already runs `trust proxy 1`). The limiter itself is covered by `tests/api/rateLimit.api.test.js`, which deliberately reuses one IP.
- **`clearMocks` resets calls but not implementations** — a `mockRejectedValue` would leak into later tests. Every unit file calls `resetMocks({ prisma, redis, ... })` in `beforeEach` (`tests/helpers/resetMocks.js`).
- **Integration/E2E databases are separate** (`goupyl_sport_test`, `goupyl_sport_e2e`) and guarded: the setup refuses to run if `DATABASE_URL` does not contain `test`/`e2e`.
- **Quota tests must stay inside one calendar month** — the quota resets on the calendar month *of the session*, so spreading bookings across days can silently cross a month boundary depending on the run date.

`vite.config.js` proxy target is overridable via `VITE_API_PROXY_TARGET` (defaults to `localhost:3000`) so E2E can run its own API on port 3100 without touching the dev setup.
